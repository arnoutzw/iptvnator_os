...
- View-level hooks: `registerBoundaryHandler` (consulted at spatial dead-ends and when nothing is focused) and `registerBackHandler` (a Back-ladder step before router back). `TvNavLiveSidebarController` (same lib) uses them for the zapping flow: while watching live TV with the channel sidebar collapsed, ← opens the sidebar and focuses the playing channel (retrying briefly for `@defer`/virtual-scroll rendering), ↑/↓ browse, OK zaps, Back closes the sidebar while focus is inside it. Wired into the M3U video player, Xtream `live-stream-layout`, and Stalker `stalker-live-stream-layout`
